...
(function (window) {
    ...
    window.Poptrox = function (selector, options) {
        ...
        if (settings.popupSelector) {
            ...
            popup.innerHTML = `
                ${settings.usePopupLoader ? `<div class="loader">${settings.popupLoaderText}</div>` : ''}
                <div class="pic"></div>
                ${settings.usePopupCaption ? `<div class="caption"></div>` : ''}
                ${settings.usePopupCloser ? `<span class="closer">${settings.popupCloserText}</span>` : ''}
                ${settings.usePopupNav ? `<div class="nav-previous"></div><div class="nav-next"></div>` : ''}
            `;
        }
        ...
        if (settings.usePopupDefaultStyling) {
            ...
            if (navNext && navPrev) {
                ...
                navNext.innerHTML = '<div style="position: absolute; height: 100px; width: 125px; top: 50%; right: 0; margin-top: -50px;">&gt;</div>';

                navPrev.style.left = '0';
                navPrev.style.width = settings.usePopupEasyClose ? '100px' : '25%';
                navPrev.innerHTML = '<div style="position: absolute; height: 100px; width: 125px; top: 50%; left: 0; margin-top: -50px;">&lt;</div>';
            }
        }
        ...
        function fadeIn(el, speed, callback) {
            ...
            void el.offsetWidth; 
            el.style.opacity = '1';
            setTimeout(() => { if(callback) callback(); }, speed);
        }
        ...
        function triggerSwitch(index) {
            ...
            function finalizeSwitch() {
                ...
                let targetWidth = contentObject.width || contentObject.naturalWidth || contentObject.offsetWidth;
                let targetHeight = contentObject.height || contentObject.naturalHeight || contentObject.offsetHeight;

                // The logic to show content
                const showContent = () => {
                    if (caption) {
                        caption.innerHTML = (item.captionText && item.captionText.length > 0) 
                            ? item.captionText 
        ...
        elements.forEach((el, index) => {
            ...
            if (!item.type) {
                ...
                switch (domain) {
                    case 'api.soundcloud.com': item.type = 'soundcloud'; break;
                    case 'youtu.be': item.type = 'youtube'; break;
                    case 'vimeo.com': item.type = 'vimeo'; break;
                    case 'wistia.net': item.type = 'wistia'; break;
                    case 'bcove.me': item.type = 'bcove'; break;
                    default: item.type = 'image'; break;
                }
            }
            ...
            switch (item.type) {
                ...
                case 'soundcloud':
                    ...
                    item.object.src = `//w.soundcloud.com/player/?url=${encodeURIComponent(item.src)}&${item.options || ''}`;
                    item.width = '600';
                    item.height = '166';
                    break;
                case 'youtube':
                    item.object = document.createElement('iframe');
                    item.object.src = `//www.youtube.com/embed/${path}${item.options ? '?' + item.options : ''}`;
                ...
                case 'vimeo':
                    item.object = document.createElement('iframe');
                    item.object.src = `//player.vimeo.com/video/${path}${item.options ? '?' + item.options : ''}`;
                    item.object.setAttribute('allowFullScreen', '1');
                    item.object.frameBorder = '0';
                    item.width = item.width || '800';
                    item.height = item.height || '480';
